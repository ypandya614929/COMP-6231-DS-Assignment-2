				...
				synchronized(playerObj) {
					if (playerObj.userName.equals(Username) && playerObj.ipAddress.equals(OldIPAddress) && playerObj.password.equals(Password)) {
						...
						if (data.trim().equals("Player created successfully")) {
							...
							logger.info("IP : " + OldIPAddress + ", username : " + Username + ", Result transferAccount() : Player account (" + OldIPAddress + ") is transfered to " + NewIPAddress);
							return "Player account (" + OldIPAddress + ") is transfered to " + NewIPAddress;
						} else {
